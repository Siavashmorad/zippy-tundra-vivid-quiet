...
function buildDeepLink(data: PushData, appRole: "seller" | "customer" = "seller"): string {
  const base = appRole === "customer" ? "/c" : "/";
  if (typeof data.url === "string" && data.url.trim()) return data.url;
  if (data.orderId) {
    const sellerOrder = data.type === "new_order" || data.type === "order.new";
    return `${sellerOrder ? "/" : base}?tab=orders&order=${encodeURIComponent(data.orderId)}`;
  }
  if (data.messageId && data.customerId) return `${base === "/" ? "" : base}?tab=messages&customer=${encodeURIComponent(data.customerId)}&message=${encodeURIComponent(data.messageId)}`;
  if (data.customerId) return `${base === "/" ? "" : base}?tab=messages&customer=${encodeURIComponent(data.customerId)}`;
  return base;
}
function openSafeUrl(raw: unknown) { if (typeof window === "undefined" || typeof raw !== "string" || !raw) return; try { const url = new URL(raw, window.location.origin); if (url.origin === window.location.origin) window.location.assign(`${url.pathname}${url.search}${url.hash}`); } catch { /* ignore malformed deep link */ } }
...
function markSeen(key: string) { if (!key) return; try { const seen = readSeen(); seen[key] = Date.now(); window.localStorage.setItem(DEDUP_KEY, JSON.stringify(seen)); } catch { /* storage unavailable */ } }
...
export async function setupNativePush(appRole: "seller" | "customer") {
  ...
  try {
    ...
    const listeners = [
      ...
      await PushNotifications.addListener("pushNotificationActionPerformed", ({ notification }) => { openSafeUrl(buildDeepLink((notification.data ?? {}) as PushData, appRole)); }),
      await LocalNotifications.addListener("localNotificationActionPerformed", ({ notification }) => { openSafeUrl(buildDeepLink((notification.extra ?? {}) as PushData, appRole)); }),
    ];
