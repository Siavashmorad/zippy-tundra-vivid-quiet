  ...
  return (
    <main className="mx-auto flex min-h-dvh max-w-md flex-col justify-center px-5 py-10">
      <div className="mb-8 text-center">
        <div className="mx-auto grid size-16 place-items-center rounded-[1.6rem] bg-emerald-600 text-white shadow-card">
          <ToranjMark className="size-10" />
        </div>
        <h1 className="mt-5 text-2xl font-semibold">مشتری ترنج</h1>
        <p className="mt-1 text-sm text-ink-soft">ثبت سفارش مستقیم از ترنج</p>
      </div>
      <div className="rounded-[1.75rem] bg-surface p-5 shadow-card">
        <div className="mb-5 grid grid-cols-2 rounded-xl bg-paper-2 p-1">
          <button
            type="button"
            className={`rounded-lg py-2.5 text-sm ${mode === "in" ? "bg-surface shadow-card" : "text-ink-soft"}`}
            onClick={() => setMode("in")}
          >
            ورود
          </button>
          <button
            type="button"
            className={`rounded-lg py-2.5 text-sm ${mode === "up" ? "bg-surface shadow-card" : "text-ink-soft"}`}
            onClick={() => setMode("up")}
          >
            ثبت‌نام
          </button>
        ...
        <form className="space-y-3" onSubmit={submit}>
          {mode === "up" ? (
            <>
              <input
                className="h-12 w-full rounded-xl border border-line bg-paper px-3"
                placeholder="نام"
                value={firstName}
          ...
          <button
            type="submit"
            disabled={busy}
            className="h-12 w-full rounded-xl bg-emerald-600 text-sm font-semibold text-white disabled:opacity-60"
          >
            {busy ? "لطفاً صبر کنید…" : mode === "up" ? "ثبت‌نام مشتری" : "ورود به حساب"}
          </button>
        </form>
      </div>
    </main>
