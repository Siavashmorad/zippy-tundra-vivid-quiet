...
export const Route = createFileRoute("/customer-login" as never)({ component: CustomerLogin });
...
function CustomerLogin() {
  ...
  const [firstName, setFirstName] = useState(""); const [lastName, setLastName] = useState(""); const [phone, setPhone] = useState(""); const [password, setPassword] = useState("");
  const [busy, setBusy] = useState(false); const [error, setError] = useState("");
  useEffect(() => { try { const saved = localStorage.getItem(LAST_KEY); if (saved) setPhone(saved); } catch {} }, []);
  useEffect(() => { if (!isPending && user) void navigate({ to: "/c" as never }); }, [isPending, user, navigate]);
  async function submit(e: React.FormEvent) {
    e.preventDefault(); setBusy(true); setError("");
    try {
      const normalized = normalizeIranPhone(phone); if (!normalized) throw new Error("شماره موبایل معتبر نیست.");
      ...
      await authClient.getSession(); await navigate({ to: "/c" as never });
    } catch (e) { setError(toFaError(e)); } finally { setBusy(false); }
  }
