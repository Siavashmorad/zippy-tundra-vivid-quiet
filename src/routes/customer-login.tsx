...
function CustomerLogin() {
  ...
  useEffect(() => { try { const saved = localStorage.getItem(LAST_KEY); if (saved) setPhone(saved); } catch { /* storage unavailable */ } }, []);
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      if (mode === "up") {
        ...
        const result = await authClient.signUp.email({ email, password, name: `${firstName.trim()} ${lastName.trim()}`, rememberMe: true });
      ...
      try { localStorage.setItem(LAST_KEY, normalized); } catch { /* storage unavailable */ }
      await authClient.getSession();
      await navigate({ to: "/c" });
    } catch (e) { setError(toFaError(e)); }
